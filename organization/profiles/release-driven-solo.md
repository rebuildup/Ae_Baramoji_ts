...
Ae_Baramoji_ts の current release-driven development で使用する Operating Model を定義します。
...
- release planning は repository で既に定義された GitHub Issue / release branch / release docs を使用する。未採用の planning service をこの PR だけで追加しない。
...
- 本 repository を TypeScript source-of-truth、`rebuildup/Ae_Baramoji` を distribution repository とする既存境界を維持する。
- Bun / build / verify / release sync の既存 commands を canonical entry point として保持する。
- submodule 反映や GitHub Release は external side effect として current candidate identity と authorization を確認する。
